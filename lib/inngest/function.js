...
export const generateMonthlyReports = inngest.createFunction(
  ...
  categories: Object.entries(stats.byCategory).map(([name, amount]) => ({
    name,
    amount,
  })),
});
  ...
  async ({ step }) => {
    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        console.log(
          `User: ${budget.user.name}, Budget: ₹${budgetAmount}, Expenses: ₹${totalExpenses} (${percentageUsed.toFixed(
            2
          )}%)`
        );
        ...
        if (
          ...
          try {
            await sendEmail({
              ...
              categories: [
                ...
                { name: "Spent So Far", amount: totalExpenses },
                { name: "Remaining", amount: budgetAmount - totalExpenses },
              ],
            });
          ...
          console.log(
            `No alert sent to ${budget.user.name} — usage below threshold or already alerted`
          );
        }
      });
    }
